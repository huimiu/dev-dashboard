  ...
  headerContent(): JSX.Element | undefined {
    return (
      <div className={widgetStyle.headerContent}>
        ...
        <Button key="bt-issue-more" icon={<MoreHorizontal32Regular />} appearance="transparent" />
      </div>
    );
  }
          ...
          this.state.issues?.map((item: githubIssuesModel, index) => {
            return (
              <div
                ...
                className={index % 2 === 0 ? "issue-item-even-layout" : "issue-item-odd-layout"}
              >
                ...
                >
                  <div>
                    <Text key={`cb-issue-state-${index}`} className="issue-state">
                      [{item.state}]&nbsp;
                    </Text>
                    {item.title}
                  </div>
                  {item.body && (
                    <Text key={`div-issue-${item.title}`} className="issue-desc">
                      {item.body}
                    </Text>
                  )}
                </div>
    ...
    if (!this.state.loading && this.state.issues?.length !== 0) {
      return (
        <Button
          ...
          onClick={() =>
            window.open(
              `https://github.com/${configs.REPOSITORY_OWNER_NAME}/${configs.REPOSITORY_NAME}/issues`,
              "_blank"
            )
    ...
    return (
      ...
      >
        ...
        <input
          ref={this.inputRef}
          type="text"
          className={this.state.inputFocused ? "input-issue-focused" : "input-issue-unfocused"}
