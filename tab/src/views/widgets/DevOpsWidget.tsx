...
export class DevOps extends Widget<IWorkItemState> {
    ...
    return (
      <div className={widgetStyle.headerContent}>
        <TeamsFxContext.Consumer>
          {({ themeString }) =>
            themeString === "default" ? (
              <Image key="icon-devops-default" width="20px" src="devops-default.svg" />
            ) : (
  ...
  protected bodyContent(): JSX.Element | undefined {
    const hasWorkItem = this.state.devOpsData !== undefined && this.state.devOpsData?.length !== 0;
    return (
          ...
          <div className="devops-list-layout">
            ...
            <div className="work-items">
              {this.state.devOpsData?.map((item: DevOpsModel, index) => {
                return (
                  <>
                    {index !== 0 && <div key={`div-items-divider-${index}`} className="divider" />}
                    <div key={`div-item-${index}`} className="work-item-layout">
                      ...
                      <Text key={`text-item-type-${index}`}>{item.fields.workItemType}</Text>
                      <div
                      ...
                      <div className="work-item-assigned-state-layout">
                        ...
                        <Text key={`text-item-state-${index}`}>{item.fields.state}</Text>
                      </div>
                    </div>
                  </>
                );
    ...
    if (!this.state.loading && this.state.devOpsData?.length !== 0) {
      return (
        <Button
          ...
          onClick={() => {
            window.open(
              `https://dev.azure.com/${configs.ORGANIZATION_NAME}/${configs.PROJECT_NAME}/_workitems/recentlyupdated/`,
              "_blank"
            );
